...
app.config['MAIL_USERNAME'] = 'YOUR USERNAME HERE'
...
def maps():
    ...
    srt_full = [((0,'NULL'),'red'),((0,'NULL'),'blue'),((0,'NULL'),'green')]
    btn = 'no_button'
    scoreform = ScoreForm()
    if scoreform.validate_on_submit():
        start = scoreform.Source.data
        end = scoreform.Destination.data
        scores = generate_url_and_scores(start,end)
        order = []
        for i,score in enumerate(scores):
            order.append((i,score))
        ...
        btn = 'button'
    return render_template('maps.html',color = color,scoreform=scoreform,srt_full=sorted(srt_full,key = lambda x:x[0][1]),btn=btn)
